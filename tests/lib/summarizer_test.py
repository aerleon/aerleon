...
class SummarizerTest(absltest.TestCase):
    ...
    def testToDottedQuad(self):
        ...
        self.assertEqual(summarizer.ToDottedQuad(net), ('192.168.0.64', '255.255.255.224'))
        net = summarizer.DSMNet(3232235584, 4294901984)
        self.assertEqual(summarizer.ToDottedQuad(net, negate=True), ('192.168.0.64', '0.0.255.31'))

        test_data = [
            (summarizer.DSMNet(3232235584, 4294967295), True, ('192.168.0.64', '32')),
            (summarizer.DSMNet(3232235584, 4294901760), True, ('192.168.0.64', '16')),
            (summarizer.DSMNet(3232235584, 4294967294), True, ('192.168.0.64', '31')),
            (summarizer.DSMNet(3232235584, 4290772992), True, ('192.168.0.64', '10')),
            (summarizer.DSMNet(3232235584, 4294966016), True, ('192.168.0.64', '255.255.251.0')),
            (summarizer.DSMNet(3232235584, 4294901504), True, ('192.168.0.64', '255.254.255.0')),
        ]
    ...
    def testInt32ToDottedQuad(self):
        self.assertEqual(summarizer._Int32ToDottedQuad(3232235584), '192.168.0.64')
    ...
    def testToPrettyBinaryFormat(self):
        # 192.168.0.64
        self.assertEqual(
            summarizer._ToPrettyBinaryFormat(3232235584), '11000000 10101000 00000000 01000000'
        )
        # 8.8.8.8
        self.assertEqual(
            summarizer._ToPrettyBinaryFormat(134744072), '00001000 00001000 00001000 00001000'
        )
        # 0.0.0.0
        self.assertEqual(
            summarizer._ToPrettyBinaryFormat(0), '00000000 00000000 00000000 00000000'
        )
        # fc00::1
        self.assertEqual(
    ...
    def testOrder(self):
        ...
        self.assertEqual(
            result,
            [summarizer.DSMNet(1249711233, 4294967039), summarizer.DSMNet(3512046465, 4294967295)],
        )
