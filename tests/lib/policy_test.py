...
class PolicyTest(parameterized.TestCase):
    ...
    def testBadPortProtocols(self):
        ...
        self.assertRaises(policy.TermPortProtocolError, policy.ParsePolicy, pol, self.naming)
    ...
    def testBadLogging(self):
        ...
        self.assertRaises(policy.InvalidTermLoggingError, policy.ParsePolicy, pol, self.naming)

    def testBadAction(self):
        pol = HEADER + BAD_TERM_7
        self.assertRaises(policy.InvalidTermActionError, policy.ParsePolicy, pol, self.naming)
    ...
    def testBadICMPTypes(self):
        ...
        self.assertRaises(policy.TermInvalidIcmpType, policy.ParsePolicy, pol, self.naming)
    ...
    def testBadProtocolEtherTypes(self):
        ...
        self.assertRaises(policy.TermProtocolEtherTypeError, policy.ParsePolicy, pol, self.naming)
    ...
    def testNextIP(self):
        ...
        self.naming.GetNetAddr.assert_has_calls([mock.call('PROD_NETWRK'), mock.call('NEXT_IP')])
    ...
    def testBadICMPCodes(self):
        ...
        self.assertRaises(policy.ICMPCodeError, policy.ParsePolicy, pol, self.naming)
        self.assertRaises(policy.ICMPCodeError, policy.ParsePolicy, pol2, self.naming)
    ...
    def testInvalidTTL(self):
        ...
        self.assertRaises(policy.InvalidTermTTLValue, policy.ParsePolicy, pol, self.naming)
    ...
    def testAddressCleanupCorrect(self):
        ...
        self.assertEqual(nacaddr.CollapseAddrList(unoptimized_addr), term.source_address)
        pol = policy.ParsePolicy(HEADER_SRX + GOOD_TERM_2, self.naming)
        term = pol.filters[0][1][0]
        self.assertEqual(
            nacaddr.CollapseAddrListPreserveTokens(unoptimized_addr), term.source_address
        )
    ...
    def testEmptyIpContains(self, mock_naming):
        ...
        term_one.AddObject(policy.VarType(2, 'accept'))
        term_two = policy.Term([policy.VarType(3, 'SMALLER_PROD'), policy.VarType(7, (22, 22))])
        term_two.AddObject(policy.VarType(2, 'accept'))
        self.assertIn(term_two, term_one)
        self.assertNotIn(term_one, term_two)
    ...
    def testIpExcludeContains(self, mock_naming):
        ...
        term_two = policy.Term([policy.VarType(3, 'FOO'), policy.VarType(11, 'BAR')])
        self.assertIn(term_two, term_one)
        self.assertNotIn(term_one, term_two)
    ...
    def testIpDualExcludeContains(self, mock_naming):
        ...
        term_one = policy.Term([policy.VarType(3, 'FOO'), policy.VarType(11, 'BAR')])
        term_two = policy.Term([policy.VarType(3, 'FOO'), policy.VarType(11, 'BAR')])
        self.assertIn(term_two, term_one)
        self.assertNotIn(term_one, term_two)
    ...
    def testTermTargetResources(self):
        ...
        term_one = policy.Term([policy.VarType(policy.VarType.TARGET_RESOURCES, target_resources)])
