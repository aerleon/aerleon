...
class NacaddrUnitTest(absltest.TestCase):
    ...
    def testAddressListExclusion(self):
        ...
        self.assertListEqual(nacaddr.AddressListExclude([a1, a2], [b1, b2, b3]), expected)
        # [1,2,3] + [4,5,6] = [1,2,3,4,5,6].  this is basically the same test as
        # above but i think it's a little more readable
        expected_two = list(a1.address_exclude(b1))
        expected_two.extend(a2.address_exclude(b2))
        self.assertListEqual(
            nacaddr.AddressListExclude([a1, a2], [b1, b2, b3]), sorted(expected_two)
        )
    ...
    def testComplexAddressListExcludesion(self):
        ...
        self.assertListEqual(nacaddr.AddressListExclude(superset, excludes), expected)
    ...
    def testCollapseAddrListPreserveTokens(self):
        addr_list = [
            nacaddr.IPv4('10.0.1.7/32', token='BIZ'),
            ...
            nacaddr.IPv4('192.168.1.7/31', token='UNDERSUPER', strict=False),
        ]
        expected = [
            nacaddr.IPv4('10.0.0.7/32', token='BAR'),
            nacaddr.IPv4('10.0.0.9/32', token='BAR'),
    ...
    def testIsSupernet(self):
        ...
        addrs5 = [nacaddr.IPv4('192.168.1.1/24', strict=False), nacaddr.IPv4('10.0.1.0/24')]
    ...
    def testSafeCollapsing(self):
        test_data = [
            (
                [nacaddr.IPv4('10.0.0.0/8'), nacaddr.IPv4('10.0.0.0/10')],
                [nacaddr.IPv4('10.0.0.0/9')],
                [nacaddr.IPv4('10.0.0.0/8'), nacaddr.IPv4('10.0.0.0/10')],
            ),
            (
                [nacaddr.IPv4('192.168.0.0/27'), nacaddr.IPv4('192.168.0.0/24')],
                [nacaddr.IPv4('192.168.1.0/24')],
                [nacaddr.IPv4('192.168.0.0/24')],
            ),
            (
                [nacaddr.IPv6('10::/56'), nacaddr.IPv6('10::/128')],
                [nacaddr.IPv6('10::/64')],
                [nacaddr.IPv6('10::/56'), nacaddr.IPv6('10::/128')],
            ),
            (
        ...
        for addresses, complement_addresses, result in test_data:
            self.assertEqual(nacaddr.CollapseAddrList(addresses, complement_addresses), result)
