...
def _collapse_mock_writes(open_mocks, key):
  return "".join(
      [str(c.args[0]) for c in list(open_mocks[key]().write.mock_calls)])
        ...
        try:
          ...
          try:
            ...
            failed_assertion = AssertionError(
                "Assertion failed: reference file {file} is not available for test {testname}"
                .format(file=file_ref, testname=func.__qualname__))
