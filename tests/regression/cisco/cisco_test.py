...
class CiscoTest(absltest.TestCase):
    ...
    @capture.stdout
    def testDSCP(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_16, self.naming), EXP_INFO)
        self.assertTrue(re.search('permit tcp any any dscp 42', str(acl)), str(acl))
        print(acl)
    ...
    def testRemark(self):
        ...
        acl = cisco.Cisco(
            policy.ParsePolicy(GOOD_STANDARD_NUMBERED_HEADER + GOOD_STANDARD_TERM_1, self.naming),
            EXP_INFO,
        )
    ...
    def testTcpEstablished(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_3, self.naming), EXP_INFO)
        self.assertTrue(re.search('permit tcp any any established\n', str(acl)), str(acl))
        print(acl)

    @capture.stdout
    def testLogging(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_4, self.naming), EXP_INFO)
        self.assertTrue(re.search('permit tcp any any log\n', str(acl)), str(acl))
        print(acl)

    @capture.stdout
    def testVerbatimTerm(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_5, self.naming), EXP_INFO)
    ...
    def testUdpEstablished(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_9, self.naming), EXP_INFO)
        self.assertFalse(re.search('permit 17 any any established', str(acl)), str(acl))
        print(acl)

    @capture.stdout
    def testIcmpTypes(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_10, self.naming), EXP_INFO)
        # echo-reply = 0
        self.assertTrue(re.search('permit icmp any any 0', str(acl)), str(acl))
        # unreachable = 3
        self.assertTrue(re.search('permit icmp any any 3', str(acl)), str(acl))
        # time-exceeded = 11
        self.assertTrue(re.search('permit icmp any any 11', str(acl)), str(acl))
        print(acl)

    @capture.stdout
    def testIcmpCode(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_19, self.naming), EXP_INFO)
        output = str(acl)
    ...
    def testIpv6IcmpTypes(self):
        acl = cisco.Cisco(
            policy.ParsePolicy(GOOD_INET6_HEADER + GOOD_TERM_11, self.naming), EXP_INFO
        )
        # echo-reply = icmp-type code 129
        self.assertTrue(re.search('permit 58 any any 129', str(acl)), str(acl))
        # destination-unreachable = icmp-type code 1
        self.assertTrue(re.search('permit 58 any any 1', str(acl)), str(acl))
    ...
    def testUnsupportedKeywordsError(self):
        ...
        # protocol-except
        self.assertRaises(aclgenerator.UnsupportedFilterError, cisco.Cisco, pol1, EXP_INFO)
        # source-prefix
        self.assertRaises(aclgenerator.UnsupportedFilterError, cisco.Cisco, pol2, EXP_INFO)

    @capture.stdout
    def testDefaultInet6Protocol(self):
        acl = cisco.Cisco(
            policy.ParsePolicy(GOOD_INET6_HEADER + GOOD_TERM_12, self.naming), EXP_INFO
        )
    ...
    def testExpiredTerm(self, mock_warn):
        _ = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + EXPIRED_TERM, self.naming), EXP_INFO)
    ...
    def testTermHopByHop(self):
        acl = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_15, self.naming), EXP_INFO)
    ...
    def testBuildTokens(self):
        pol1 = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_5, self.naming), EXP_INFO)
        st, sst = pol1._BuildTokens()
        self.assertEqual(st, SUPPORTED_TOKENS)
        self.assertEqual(sst, SUPPORTED_SUB_TOKENS)

    def testBuildWarningTokens(self):
        pol1 = cisco.Cisco(policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_17, self.naming), EXP_INFO)
        st, sst = pol1._BuildTokens()
        self.assertEqual(st, SUPPORTED_TOKENS)
        self.assertEqual(sst, SUPPORTED_SUB_TOKENS)
    ...
    def testProtoInts(self):
        pol = policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_7 + GOOD_TERM_9, self.naming)
        acl = cisco.Cisco(pol, EXP_INFO)
        self.assertIn('permit 112 any any', str(acl), str(acl))
        self.assertIn('permit tcp any any range 1024 65535 ' 'established', str(acl), str(acl))
        self.assertIn('permit udp any any range 1024 65535', str(acl), str(acl))
        print(acl)
    ...
    def testLongHeader(self):
        pol = policy.ParsePolicy(LONG_VERSION_HEADER + GOOD_TERM_7, self.naming)
        acl = cisco.Cisco(pol, EXP_INFO)
        print(acl)
        self.assertIn('remark This long header should be split even on a', str(acl))
        self.assertIn(
            ('remark looooooooooooooooooooooooooonnnnnnnnnnnnnnnnnn' 'gggggggggg string.'),
            str(acl),
        )
        self.assertIn(
            ('remark https://www.google.com/maps/place/1600+Amphitheatr' 'e+Parkway,+Mountain+'),
            str(acl),
        )
        self.assertIn(
            ('remark View,+CA/@37.507491,-122.2540443,15z/data=!4m5!3m4!' '1s0x808fb99f8c51e88'),
            str(acl),
        )
