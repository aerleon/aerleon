...
class JuniperSRXTest(absltest.TestCase):
    ...
    def testHeaderApplyGroups(self):
        ...
        output = str(junipersrx.JuniperSRX(pol, EXP_INFO))
        self.assertIn('apply-groups [ tcp-test1 tcp-test2 ]', output, output)
        print(output)
    ...
    def testHeaderApplyGroupsExcept(self):
        ...
        output = str(junipersrx.JuniperSRX(pol, EXP_INFO))
        self.assertIn('apply-groups-except [ tcp-test1 tcp-test2 ]', output, output)
        print(output)
    ...
    def testIcmpTypes(self):
        ...
        self.assertIn('application test-icmp-app {', output, output)
        self.assertIn('term t1 protocol icmp icmp-type 0 inactivity-timeout 60', output, output)
        self.assertIn('term t2 protocol icmp icmp-type 8 inactivity-timeout 60', output, output)
        print(output)
    ...
    def testLoggingBothAccept(self):
        srx = junipersrx.JuniperSRX(
            policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_LOG_1, self.naming), EXP_INFO
        )
        output = str(srx)
    ...
    def testLoggingBothDeny(self):
        srx = junipersrx.JuniperSRX(
            policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_LOG_2, self.naming), EXP_INFO
        )
        output = str(srx)
        self.assertIn('session-init;', output)
        self.assertIn('session-close;', output)
    ...
    def testLoggingTrueAccept(self):
        srx = junipersrx.JuniperSRX(
            policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_LOG_3, self.naming), EXP_INFO
        )
        output = str(srx)
        self.assertIn('session-close;', output)
    ...
    def testLoggingTrueDeny(self):
        srx = junipersrx.JuniperSRX(
            policy.ParsePolicy(GOOD_HEADER + GOOD_TERM_LOG_4, self.naming), EXP_INFO
        )
        output = str(srx)
        self.assertIn('session-init;', output)
    ...
    def testBadICMP(self):
        ...
        self.assertRaises(
            aclgenerator.UnsupportedFilterError, junipersrx.JuniperSRX, pol, EXP_INFO
        )
    ...
    def testGlobalPolicyHeader(self):
        pol = policy.ParsePolicy(GOOD_HEADER_10 + MULTIPLE_PROTOCOLS_TERM, self.naming)
        output = str(junipersrx.JuniperSRX(pol, EXP_INFO))
    ...
    def testBadGlobalPolicyHeaderZoneBook(self):
        pol = policy.ParsePolicy(BAD_HEADER_3 + MULTIPLE_PROTOCOLS_TERM, self.naming)
    ...
    def testBadGlobalPolicyHeaderNameAll(self):
        pol = policy.ParsePolicy(BAD_HEADER_4 + MULTIPLE_PROTOCOLS_TERM, self.naming)
        self.assertRaises(junipersrx.UnsupportedFilterError, junipersrx.JuniperSRX, pol, EXP_INFO)
    ...
    def testExpiredTerm(self, mock_warn):
        _ = junipersrx.JuniperSRX(
            policy.ParsePolicy(GOOD_HEADER + EXPIRED_TERM_1, self.naming), EXP_INFO
        )
    ...
    def _AssertOrder(self, strings, expected_order):
        ...
        self.fail(
            'Strings weren\'t in expected order.\nExpected:\n  %s\n\nGot:\n%s'
            % ('\n  '.join(expected_order), '\n'.join(strings))
        )
    ...
    def testLargeTermSplittingV6(self):
        ips = list(nacaddr.IP('2620:0:1000:3103:eca0:2c09:6b32:e000/119').subnets(new_prefix=128))
        mo_ips = []
        counter = 0
        for ip in ips:
            if counter % 2 == 0:
    ...
    def testPlatformExclude(self):
        ...
        output = str(junipersrx.JuniperSRX(pol, EXP_INFO))
        self.assertIn('good_term_19', output, output)
    ...
    def testMixedVersionIcmp(self):
        pol = policy.ParsePolicy(GOOD_HEADER + ICMP_TYPE_TERM_1 + IPV6_ICMP_TERM, self.naming)
        output = str(junipersrx.JuniperSRX(pol, EXP_INFO))
        self.assertIn('term t6 protocol icmp6 icmp6-type 129 ' 'inactivity-timeout 60;', output)
        self.assertIn('term t1 protocol icmp icmp-type 0 ' 'inactivity-timeout 60;', output)
        print(output)
