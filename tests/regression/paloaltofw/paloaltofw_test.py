...
class PaloAltoFWTest(absltest.TestCase):
    ...
    def testServiceMap(self):
        ...
        self.assertEqual(
            ...
            {
                ((), ('22',), 'tcp'): {'name': 'service-ssh-term-1-tcp'},
                ((), ('25',), 'tcp'): {'name': 'service-smtp-term-1-tcp'},
            },
            pol1.service_map.entries,
        )
        print(pol1)
        ...
        self.assertEqual(pol2.service_map.entries, expected_entries, pol2.service_map.entries)
        print(pol2)
    ...
    def testIcmpTypes(self):
        ...
        self.assertCountEqual(
            ['icmp-echo-reply', 'icmp-echo-request', 'icmp-unreachable'], members, output
        )
        print(output)
    ...
    def testIcmpTypesMultiplePolicies(self):
        pol = policy.ParsePolicy(
            GOOD_HEADER_1 + ICMP_TYPE_TERM_1 + GOOD_HEADER_2 + ICMP_TYPE_TERM_2, self.naming
        )
        paloalto = paloaltofw.PaloAltoFW(pol, EXP_INFO)
        output = str(paloalto)
        x = paloalto.config.find(f"{PATH_RULES}/entry[@name='test-icmp']/application")
    ...
    def testIcmpV6Types(self):
        ...
        self.assertCountEqual(
            ['icmp6-echo-reply', 'icmp6-echo-request', 'icmp6-destination-unreachable'],
            members,
            output,
        )
        print(output)
    ...
    def testIcmpV6ApplicationAbbreviation(self):
        pol = policy.ParsePolicy(GOOD_HEADER_MIXED + ICMPV6_ABBREVIATION_TYPE_TERM, self.naming)
        paloalto = paloaltofw.PaloAltoFW(pol, EXP_INFO)
        output = str(paloalto)
        ...
        self.assertCountEqual(
            [
                ...
                'icmp6-INV-NBR-DSCVR-SOL',
                'icmp6-INV-NBR-DSCVR-ADV',
                'icmp6-version-2-MCAST-LSNR-repo',
...
"""
        ...
        self.assertRaises(paloaltofw.UnsupportedFilterError, paloaltofw.PaloAltoFW, pol, EXP_INFO)
...
"""
        self.assertRaises(
            policy.TermInvalidIcmpType, policy.ParsePolicy, POL % ("inet6", T), self.naming
        )

    def testBadICMPv6Type(self):
        pol = policy.ParsePolicy(GOOD_HEADER_MIXED + BAD_ICMPV6_TYPE_TERM, self.naming)
        self.assertRaises(
            paloaltofw.PaloAltoFWBadIcmpTypeError, paloaltofw.PaloAltoFW, pol, EXP_INFO
        )
    ...
    def testZoneLen(self):
        ...
        self.assertRaisesRegex(
            paloaltofw.PaloAltoFWNameTooLongError,
    ...
    def test_ZonesRequired(self):
        ...
        msg = '^Palo Alto Firewall filter arguments ' 'must specify from-zone and to-zone[.]$'
        for header in BAD_HEADERS:
            pol = policy.ParsePolicy(header + GOOD_TERM_3, self.naming)
            self.assertRaisesRegex(
                paloaltofw.UnsupportedFilterError, msg, paloaltofw.PaloAltoFW, pol, EXP_INFO
            )
...
term rule-1 {
  ...
  action:: accept
        ...
        # maximum length
        pol = policy.ParsePolicy(
            POL % ('C' * MAX_TAG_COMMENTS_LENGTH, 'C' * MAX_RULE_DESCRIPTION_LENGTH), self.naming
        )
        paloalto = paloaltofw.PaloAltoFW(pol, EXP_INFO)
        output = str(paloalto)
        print(output)
        ...
        # maximum length + 1
        pol = policy.ParsePolicy(
            POL % ('C' * (MAX_TAG_COMMENTS_LENGTH + 1), 'C' * (MAX_RULE_DESCRIPTION_LENGTH + 1)),
            self.naming,
        )
        paloalto = paloaltofw.PaloAltoFW(pol, EXP_INFO)
...
term rule-1 {
  ...
  action:: accept
        ...
        for i, app in enumerate(APPS):
            pol = policy.ParsePolicy(POL2 % (' '.join(app), 'protocol:: tcp'), self.naming)
            paloalto = paloaltofw.PaloAltoFW(pol, EXP_INFO)
            output = str(paloalto)
            print(output)
            x = paloalto.config.findall(f"{PATH_RULES}/entry[@name='rule-1']/application/member")
        ...
        self.assertRaisesRegex(
            paloaltofw.UnsupportedFilterError, regex, paloaltofw.PaloAltoFW, pol, EXP_INFO
        )
        ...
        self.assertRaisesRegex(
            ...
            '^Term rule-1 contains mixed uses of protocols ' 'with and without port numbers',
            policy.ParsePolicy,
            POL6,
            definitions,
        )
...
"""
        ...
        addrs = {elem.text for elem in x}
        self.assertEqual(
            {"10.3.1.0/24", "10.3.2.0/24", "2001:db8:0:aa::/64", "2001:db8:0:bb::/64"},
            addrs,
            output,
        )
        T = """
