...
class NftablesTest(parameterized.TestCase):
    ...
    @parameterized.parameters(
        ...
        (
            'inet',
            ['9782:b30a:e5c6:1aa4:29ff:e57c:44a0:1b84'],
            ['200.1.1.3/32', '2606:4700:4700::1111'],
            [
                'ip6 saddr 9782:b30a:e5c6:1aa4:29ff:e57c:44a0:1b84/128 ip6 daddr 2606:4700:4700::1111/128'
            ],
    ...
    def testGroupExpressions(self, address_expr, porst_proto_expr, opt, verdict, expected_output):
    ...
    def testDuplicateTerm(self):
        ...
        with self.assertRaises(nftables.TermError):
            nftables.Nftables.__init__(nftables.Nftables.__new__(nftables.Nftables), pol, EXP_INFO)
    ...
    def testGroupExpressionsNoMixedAFCrossPairing(self):
        ...
        for rule in result:
            ...
            self.assertFalse(
                'ip6 saddr' in rule and rule.lstrip().startswith('ip6') and 'icmp type' in rule and 'icmpv6' not in rule,
    ...
    def testGoodHeader(self):
        nftables.Nftables(policy.ParsePolicy(GOOD_HEADER_1 + GOOD_TERM_1, self.naming), EXP_INFO)
        nft = str(
            nftables.Nftables(
                policy.ParsePolicy(
                    GOOD_HEADER_1 + GOOD_TERM_1 + GOOD_HEADER_2 + IPV6_TERM_2, self.naming
                ),
                EXP_INFO,
            )
        )
    ...
    @parameterized.parameters(
        ...
        (['0.0.0.0/8', '2001:db8::/32'], {'ip': ['0.0.0.0/8'], 'ip6': ['2001:db8::/32']}),
    )
    ...
    @parameterized.parameters(
        (
            ESTABLISHED_OPTION_TERM,
            'WARNING: Term established-term is a established term and will not be rendered.',
        ),
        (
            TCP_ESTABLISHED_OPTION_TERM,
            'WARNING: Term tcp-established-term is a tcp-established term and will not be rendered.',
        ),
    )
