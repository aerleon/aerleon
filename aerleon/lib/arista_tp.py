...
class Term(aclgenerator.Term):
    ...
    def __str__(self):
        ...
        is_default_term = re.match(r"^ipv(4|6)\-default\-.*", self.term.name, re.IGNORECASE)
                ...
                for ft in ftypes:
                    ...
                    if term.option:
                        ...
                        for opt in [str(x) for x in term.option]:
                            if opt.startswith("sample") or opt.startswith("first-fragment"):
                    ...
                    has_unsupported_match_criteria = (
                        ...
                        or term.dscp_match
                        or term.ether_type
                        or term.flexible_match_range
                        or term.forwarding_class
                        or term.forwarding_class_except
                        or term.next_ip
                        or term.port
                        or term.traffic_type
                    )
                    ...
                    if term.source_address_exclude:
                        src_addr = term.GetAddressOfVersion("source_address", self._AF_MAP[ft])
