...
class Term:
    ...
    def NormalizeIcmpTypes(self, icmp_types, protocols, af):
        ...
        if (af != 4 and protocols == ['icmp']) or (af != 6 and protocols == ['icmpv6']):
        ...
        for icmptype in icmp_types:
            if icmptype not in self.ICMP_TYPE[af]:
                raise UnknownIcmpTypeError(
                    '%s %s %s %s'
                    % (
                        '\nUnrecognized ICMP-type (',
                        icmptype,
                        ') specified in term ',
                        self.term.name,
                    )
        ...
        if af in self._FILTER_BLACKLIST:
            ...
            if unsupported_protocols:
                raise UnsupportedFilterError(
                    ...
                    % (self._PLATFORM, unsupported_protocols, af, term.name)
                )
...
def AddRepositoryTags(prefix='', rid=True, date=True, revision=True, wrap=False):
    ...
    if rid:
        tags.append(f'{prefix}{wrapper}$Id:${wrapper}')
    if date:
        tags.append(f'{prefix}{wrapper}$Date:${wrapper}')
    if revision:
        tags.append(f'{prefix}{wrapper}$Revision:${wrapper}')
    return tags
...
def WrapWords(textlist, size, joiner='\n'):
    ...
    linelength_re = re.compile(
        r'(\S{%d}(?!\s|\Z)|(?<=\S{%d})\S+|\S.{1,%d}(?=\s|\Z))' % (size, size, size - 1)
    )
