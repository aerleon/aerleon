...
def CollapseAddrListPreserveTokens(addresses: List[IPv4]) -> List[IPv4]:
    ...
    """
    ret_array = []
    for grp in itertools.groupby(
        sorted(addresses, key=lambda x: x.parent_token), lambda x: x.parent_token
    ):
    ...
    for addr in addresses:
        ...
        while addr_is_fresh:
            ...
            if not _SafeToMerge(addr, prev_addr, complements_by_network):
                ...
                prev_addr.version == addr.version
...
def AddressListExclude(
    ...
    while superset and excludes:
        if superset[-1].overlaps(excludes[-1]):
            ip = superset.pop()
            superset.extend(reversed(RemoveAddressFromList([ip], excludes[-1])))
        elif (
            superset[-1]._get_networks_key() < excludes[-1]._get_networks_key()
        ):  # pylint: disable=protected-access
