...
class Term(gcp.Term):
    ...
    ACTION_MAP = {'accept': 'allowed', 'deny': 'denied'}
    ...
    def __init__(self, term, inet_version='inet', policy_inet_version='inet'):
        ...
        # account.
        if (
            not self.term.source_address and not self.term.source_tag
        ) and self.term.direction == 'INGRESS':
            raise GceFirewallError(
                'GCE firewall needs either to specify source address or source tags.'
            )
        if self.term.source_port:
            raise GceFirewallError('GCE firewall does not support source port restrictions.')
        if (
    ...
    def _validateDirection(self):
        if self.term.direction == 'INGRESS':
            ...
            if self.term.destination_address:
                raise GceFirewallError('Ingress rules cannot include ' '"destinationRanges.')

        elif self.term.direction == 'EGRESS':
            if self.term.source_address:
                raise GceFirewallError('Egress rules cannot include "sourceRanges".')

            if not self.term.destination_address:
                raise GceFirewallError('Egress rule missing required field "destinationRanges".')
    ...
    def ConvertToDict(self):
        ...
        if not self.term.protocol:
            ...
            logging.info(
                ...
                'which is interpreted as "all" protocols.',
                self.term.name,
            )
...
class GCE(gcp.GCP):
    ...
    def _BuildTokens(self):
        ...
        # add extra things
        supported_tokens |= {'destination_tag', 'expiration', 'owner', 'priority', 'source_tag'}
        ...
        for header, terms in pol.filters:
            ...
            for opt in filter_options:
                try:
                    ...
                    logging.info('Checking policy for max attribute count %d', max_attribute_count)
            ...
            if IsDefaultDeny(terms[-1]):
                ...
                if direction == 'EGRESS':
                    ...
                    if address_family != 'mixed':
                        ...
                        terms[-1].source_address = [self._ANY_IP['inet'], self._ANY_IP['inet6']]
            ...
            for term in terms:
                ...
                for term_af in term_address_families:
                    for rules in Term(term, term_af, address_family).ConvertToDict():
                        logging.debug(
                            'Attribute count of rule %s is: %d',
                            term.name,
...
def GetAttributeCount(dict_term: Dict[str, Any]) -> int:
    ...
    """
    addresses = len(dict_term.get('destinationRanges', [])) or len(
        dict_term.get('sourceRanges', [])
    )
