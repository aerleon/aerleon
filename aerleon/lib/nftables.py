...
class Term(aclgenerator.Term):
        ...
        if src_addr and dst_addr:
            ...
            if address_family == 'inet' or address_family == 'ip6':
                if src_addr_book['ip6'] and dst_addr_book['ip6']:
                    address_statement.append(
                        'ip6 saddr '
                        + self.CreateAnonymousSet(src_addr_book['ip6'])
                        + ' '
                        + 'ip6 daddr '
    ...
    def RulesetGenerator(self, term: policy.Term) -> list[str]:
        ...
        proto_and_ports = self.PortsAndProtocols(
            self.address_family,
            self.term.protocol,
        ...
        for header, terms in pol.filters:
            ...
            for term in terms:
                ...
                if 'established' in term.option:
                    logging.warning(
                        'WARNING: Term %s is a established ' 'term and will not be rendered.',
                        term.name,
                    )
                    continue
                if 'tcp-established' in term.option:
                    logging.warning(
                        'WARNING: Term %s is a tcp-established ' 'term and will not be rendered.',
                        term.name,
                    )
                    continue
