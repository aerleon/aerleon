...
class Term(iptables.Term):
    ...
    def _CalculateAddresses(
        ...
        src_addr_list = self._CalculateAddrList(
            src_addr_list, src_addr_exclude_list, target_af, 'src'
        )
        dst_addr_list = self._CalculateAddrList(
            dst_addr_list, dst_addr_exclude_list, target_af, 'dst'
        )
...
class Ipset(iptables.Iptables):
    ...
    def _GenerateSetConfig(self, term: Term) -> list[str]:
        ...
        for direction in sorted(term.addr_sets, reverse=True):
            ...
            output.append(
                '%s %s %s family %s hashsize %i maxelem %i'
